...
// App Id for D2D where there is no IP address
...
// In order to calculate for device to device communication
struct UeInfo
{
    bool init;
    double txPwr;
...
MacNodeId ctrlInfoToUeId(UserControlInfo * info);
MacCid idToMacCid(MacNodeId nodeId, LogicalCid lcid);
MacCid ctrlInfoToMacCid(UserControlInfo * info);        // get the CID from the packet control info
