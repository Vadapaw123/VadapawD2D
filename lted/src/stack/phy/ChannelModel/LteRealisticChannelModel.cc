    ...
    Coord coord)
{
    ...
    EV<<"Square distance between enb to Ue is "<<sqrDistance;
...
//    }
    ...
    if ((enableExtCellInterference_ || enableMultiCellInterference_) && dir == DL)
    {
        ...
        {
            //               (      mW            +  mW  +        mW            )
            den = linearToDBm(totN + multiCellInterference[i]);

            EV << "\t ext["  << "] - multi[" << multiCellInterference[i] << "] - recvPwr["
...
double LteRealisticChannelModel::computeIndoor(double d, MacNodeId nodeId)
{
    ...
    EV<<"Carrier Frequency"<<carrierFrequency_;
...
//        a = 36.8;
//        b = 43.8;
//        int nwallss = 1; // number of the walls between two devices
//        return 36.8*log10(d)+43.8+(5(nwallss-1));
//
//    }
//    EV<<"Carrier Frequency"<<carrierFrequency_;
//    return a * log10(d) + b + 20 * log10(carrierFrequency_);
