    ...
    LteFeedbackPkt* pkt)
{
    ...
    double mean = 0;
    double max  = *std::max_element(snr.begin(),snr.end());
   // for(std::vector<double>::iterator it = snr.begin();it!= snr.end();it++)
    //{
   ...
   // mean  = mean/snr.size();
    ...
    EV<<"SINR value in the vector form "<<max;
    tSample_->id_=lteinfo->getSourceId();
    tSample_->sample_=max;
    tSample_->module_=getMacByMacNodeId(lteinfo->getSourceId());
    emit(SINR_dl_,tSample_);
    emit(SINR_dlvect_,max);
        ...
        dir = ((dir == UL )? DL : UNKNOWN_DIRECTION))
    {
        ...
        if (dir == UL)
        {
            ...
            EV<<"THis is CQI for uplink w.r.t to master";
...
//
//            }
            double max_ul  = *std::max_element(snr.begin(),snr.end());

//            mean  = mean/snr.size();
//            EV<<"MEAN of SINR"<<mean;
//
            tSample_->id_=lteinfo->getSourceId();
            tSample_->sample_=max_ul;
            tSample_->module_=getMacByMacNodeId(lteinfo->getSourceId());
            emit(SINR_ul_,tSample_);
            emit(SINR_ulvect_,mean);
